...
Compared with the built-in `HexagonalArchitectureTest`, the Spring companion adds focused test classes.

- `SpringHexagonalArchitectureTest` checks stereotype placement, repository access, dependency direction, and framework leakage.
- `PortContractTest` reinforces interface-only port contracts and domain-safe signatures.
- `DependencyDirectionTest` adds Spring wiring direction rules around application and adapter boundaries.
- `DomainIsolationTest` strengthens framework isolation in domain and application-service packages.
