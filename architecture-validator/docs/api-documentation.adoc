...
Generated built-in tests always use a non-empty `basePackage` scope.
When `basePackage` is not provided explicitly, the task infers it from the `src/main/java` package layout and then falls back to the project group.
