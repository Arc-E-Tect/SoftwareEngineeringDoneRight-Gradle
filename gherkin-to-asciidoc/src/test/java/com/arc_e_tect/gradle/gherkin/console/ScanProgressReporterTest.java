...
class ScanProgressReporterTest {
    ...
    void emitsNothingBeforeItemCountThrottleIsReached() {
        ScanProgressReporter reporter =
                new ScanProgressReporter(logger, "Parsing", 100, 5, 3600, fixedClock(0));
    ...
    void emitsStatusLineOnceItemCountThrottleIsReached() {
        ...
        assertThat(logger.lifecycleMessages()).containsExactly("Parsing: 5/100 (5%)");
    }
    ...
    void emitsNothingBeforeElapsedTimeThrottleIsReached() {
        ...
        ScanProgressReporter reporter =
                new ScanProgressReporter(logger, "Parsing", 100, 1_000_000, 2, nanos::get);
    ...
    void emitsStatusLineOnceElapsedTimeThrottleIsReached() {
        ...
        assertThat(logger.lifecycleMessages()).containsExactly("Parsing: 1/100 (1%)");
    }
    ...
    void completeEmitsFinalLineDespiteThrottleWindow() {
        ScanProgressReporter reporter =
                new ScanProgressReporter(logger, "Parsing", 100, 50, 3600, fixedClock(0));

        reporter.step();
        reporter.complete();

        assertThat(logger.lifecycleMessages()).containsExactly("Parsing: done, 1 item(s)");
    }
    ...
    void zeroTotalReporterEmitsNothingUntilComplete() {
        ScanProgressReporter reporter = ScanProgressReporter.determinate(logger, "Parsing", 0);

        reporter.complete();

        assertThat(logger.lifecycleMessages()).containsExactly("Parsing: done, 0 item(s)");
    }
    ...
    void indeterminateModeReportsRunningCount() {
        ScanProgressReporter reporter =
                new ScanProgressReporter(logger, "Scanning glue code", -1, 3, 3600, fixedClock(0));

        for (int i = 0; i < 3; i++) {
            reporter.step();
        }

        assertThat(logger.lifecycleMessages()).containsExactly("Scanning glue code: 3 processed so far");
    }
    ...
    void indeterminateCompleteLineMatchesDeterminateWording() {
        ScanProgressReporter reporter = ScanProgressReporter.indeterminate(logger, "Scanning glue code");

        reporter.step();
        reporter.step();
        reporter.complete();

        assertThat(logger.lifecycleMessages()).contains("Scanning glue code: done, 2 item(s)");
    }
    ...
    void stepWithDetailAppendsDetailToEmittedLine() {
        ScanProgressReporter reporter =
                new ScanProgressReporter(logger, "Parsing", 100, 1, 3600, fixedClock(0));

        reporter.step("UserLogin.feature");

        assertThat(logger.lifecycleMessages()).containsExactly("Parsing: 1/100 (1%) - UserLogin.feature");
    }
