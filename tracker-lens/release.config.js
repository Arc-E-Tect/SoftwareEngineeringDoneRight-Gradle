...
module.exports = {
    ...
    // The shared release/first-release-patch-analyzer.js below forces the release type to
    // "patch" whenever no prior release exists for this tag namespace, so the first real
    // release computes as exactly 0.0.1 regardless of the triggering commit's type - Shadow's
    // history shows relying on commit-analyzer's own defaults is not enough: a feat-typed first
    // commit computes 0.1.0 without this.
    tagFormat: "tracker-lens-v${version}",
    plugins: [
